...
describe('parseEnv', () => {
  it('parses all TrackFlow environment variables', () => {
    const env = parseEnv(validEnv);

    expect(env.SMTP_PORT).toBe(1025);
    expect(env.GEOIP_TIMEOUT_MS).toBe(750);
    expect(env.PUBLIC_SHORT_URL_BASE).toBe('http://localhost:3000');
    expect(env.PUBLIC_API_BASE_URL).toBe('http://localhost:3000');
  });
});
